...
[$params, $providers] = eQual::announce([
    'description'   => 'Fetches the Holidays (legal days-off) from Datto AutoTask API and returns it as a JSON array.',
    'response'      => [
...
/*
    ...
    By convention, holidays are stored in the default holiday set (ID = 1)
    ```
    {
        "id": 1,
        "holidaySetDescription": "Holiday Sets",
        "holidaySetName": "Headquarters Holidays"
    }
    ```

    ## Request

    GET https://webservices19.autotask.net/atservicesrest/v1.0/HolidaySets/1/Holidays
...

    {
        "items": [
            {
                "id": 29683373,
                "holidayDate": "2019-12-25T00:00:00Z",
                "holidayName": "Christmas / Noël",
                "holidaySetID": 1
            },
            {
                [...]
...
$url = $entrypoint_url.'HolidaySets/1/Holidays';
...
while($i < $max_loop) {
    ...
    */
    ...
    if($code != 200) {
        ...
        throw new Exception("request to AT (holidays) rejected with code $code ($status)", QN_ERROR_INVALID_PARAM);
    }
