...
function contractika_migration_task_seed(): array {
    $json = json_decode(file_get_contents('packages/contractika/init/data/core_Task.json'), true);
    $tasks = [];
...
function contractika_migration_controller_path(string $type, string $controller): string {
...
function contractika_migration_controller_params(string $content): array {
...
function contractika_migration_controller_surface(string $type, string $controller): array {
    $path = contractika_migration_controller_path($type, $controller);
    ...
    return [
        ...
        'params'          => contractika_migration_controller_params($content),
        'calls'           => array_map(
            fn($call) => [
                'type'       => $call[1],
                'controller' => $call[2],
                'path'       => contractika_migration_controller_path($call[1], $call[2]),
                'resolves'   => (strpos($call[2], 'contractika_') !== 0) || is_file(contractika_migration_controller_path($call[1], $call[2]))
            ],
            $calls
        )
    ];
...
function contractika_migration_operation_graph(array $entrypoints): array {
    $queue = $entrypoints;
    $surface = [];
    ...
    while(count($queue)) {
        ...
        $surface[$key] = contractika_migration_controller_surface($type, $controller);
...
$tests = [
    '3001' => [
        ...
        'act'         => fn() => contractika_migration_operation_graph(contractika_migration_entrypoints()),
    ...
    '3002' => [
        ...
        'act'         => function () {
            $tasks = contractika_migration_task_seed();
            $controllers = [];
            foreach($tasks as $task) {
                $controllers[$task['controller']] = contractika_migration_controller_surface('do', $task['controller']);
            }
            return [
                'tasks'       => $tasks,
    ...
    '3003' => [
        ...
        'act'         => function () {
            ...
            foreach(contractika_migration_param_contracts() as [$type, $controller, $params]) {
                $surfaces[$type . ':' . $controller] = contractika_migration_controller_surface($type, $controller);
            }
            return $surfaces;
        },
