...
@Component({
  selector: 'serviceaccountline',
  templateUrl: 'serviceaccountline.component.html',
  styleUrls: ['serviceaccountline.component.scss']
})
export class SALineComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    private default_descriptor: any = {
        ...
        context: {
            entity: 'contractika\\SALine',
            view:   'form.default'
        }
    };


    private sa_line_id: number = 0;
    ...
    public ngOnDestroy() {
        console.debug('SALineComponent::ngOnDestroy');
        this.ngUnsubscribe.next();
        this.ngUnsubscribe.complete();
    }

    public ngAfterViewInit() {
        console.debug('SALineComponent::ngAfterViewInit');

        this.context.setTarget('#sb-container-saline');
        ...
        // if(!Object.keys(descriptor.context).length) {
            this.default_descriptor.context.domain = ["id", "=", this.sa_line_id];
            this.context.change(this.default_descriptor);
        //}
    }

    public ngOnInit() {
        console.debug('SALineComponent::ngOnInit');
        ...
        */
        this.route.params.pipe(takeUntil(this.ngUnsubscribe)).subscribe( async (params) => {
            this.sa_line_id = <number> parseInt(params['sa_line_id'], 10);
            if(this.ready) {
                this.default_descriptor.context.domain = ["id", "=", this.sa_line_id];
