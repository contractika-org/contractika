...
class Report extends \equal\orm\Model {
    ...
    public static function getLink() {
        return "/contractika/#/report/object.id";
    }
    ...
    public static function getColumns() {
        ...
        return [
            ...
            'pdf_data' => [
                'type'              => 'computed',
                'result_type'       => 'binary',
                'description'       => 'Raw data of of the generated PDF (does not change over time).',
    ...
    public static function onupdateStatus($self, $values) {
        if(isset($values['status']) && in_array($values['status'], ['released', 'archived', 'sent'])) {
            $self->read(['service_account_id', 'sa_lines_ids']);
            foreach($self as $report) {
                try {
                    // update all lines attached to the report
                    SALine::ids($report['sa_lines_ids'])->update(['is_locked' => true]);
    ...
    public static function calcHasLines($self) {
        $result = [];
        $self->read(['sa_lines_ids']);
        foreach($self as $id => $report) {
            $result[$id] = (bool) count((array) $report['sa_lines_ids']);
        }
        return $result;
        // return array_map(fn ($a) => (bool) count((array) $a['sa_lines_ids']), $self->read(['sa_lines_ids'])->get());
    }

    public static function calcTotalPoints($self) {
        $result = [];
        $self->read(['sa_lines_ids' => ['has_ticket', 'has_task', 'points']]);
        foreach($self as $id => $report) {
            $total_points = 0.0;
            foreach($report['sa_lines_ids'] as $line) {
    ...
    public static function calcBalanceNew($self) {
        ...
        foreach($self as $id => $report) {
            $result[$id] = round($report['balance_old'] + $report['total_points'] + $report ['total_credits'], 2);
        }
        return $result;
    }
    ...
    public static function calcLink($self) {
        $result = [];
        foreach($self as $id => $report) {
            $result[$id] = '/?get=contractika_print-report&id='.$id;
        }
        return $result;
    }
    ...
    public static function calcHasNonPosted($self) {
        $result = [];
        $self->read(['sa_lines_ids' => ['sa_line_class_id', 'is_posted']]);
        foreach($self as $id => $report) {
            $result[$id] = false;
            foreach($report['sa_lines_ids'] as $line) {
    ...
    public static function calcIsEmpty($self) {
        ...
        $self->read(['sa_lines_ids']);
        foreach($self as $id => $report) {
            $result[$id] = empty($report['sa_lines_ids']);
        }
        return $result;
    }
    ...
    public static function calcCustomerId($self) {
        $result = [];
        $self->read(['service_account_id' => ['customer_id']]);
        foreach($self as $id => $report) {
            $result[$id] = $report['service_account_id']['customer_id'];
        }
        return $result;
    }
    ...
    public static function generatePdf($id, $params=[]) {
        ...
        $report = self::id($id)
            ->read([
                ...
                'sa_lines_ids' => [
                    ...
                    'points',
                    'calculation_log',
                    'sa_line_type_id'   => ['externalNumber'],
        ...
        $img_path = QN_BASEDIR.'/packages/contractika/views/logo_netika_sm.png';
        ...
        try {
            ...
            foreach($report['sa_lines_ids'] as $line) {
                ...
                if($line['has_ticket'] || $line['has_task']) {
                    ...
                    $who = $line['employee_id']['partner_identity_id']['firstname'].' '.substr($line['employee_id']['partner_identity_id']['lastname'], 0, 1).'.';

                    // adapt line and add to map
                    $map_tt_lines[$parent_id]['lines'][] = [
                        'date'          => date('d/m/Y', $line['date']),
                        'start'         => self::_getStringFromTime($line['start'] + $tz_offset - strtotime('midnight', $line['start'])),
                        'end'           => self::_getStringFromTime($line['end'] + $tz_offset - strtotime('midnight', $line['end'])),
                        'pause'         => self::_getStringFromTime(floor(abs($line['pause']) * 60) * 60),
                        'on_site'       => ($line['on_site'])?'Yes':'No',
                        'who'           => $who,
                        'role'          => $line['role_id']['name'],
                        'type'          => $line['sa_line_type_id']['externalNumber'],
                ...
                // line relates to a Credit or a Correction
                    ...
                    $who = $line['creator']['firstname'].' '.substr($line['creator']['lastname'], 0, 1).'.';
                    $map_cc_lines[] = [
                        'date'          => date('d/m/Y', $line['date']),
                        'who'           => $who,
            ...
            $values = [
                ...
                'service_account'   => $report['service_account_id']['name'].' ['. $report['service_account_id']['extref_at_id'] .']',
                ...
                'customer'          => $report['service_account_id']['customer_id']['name'].' ['. $report['service_account_id']['customer_id']['extref_at_id'].']',
